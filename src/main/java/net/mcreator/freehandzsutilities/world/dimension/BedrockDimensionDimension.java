...
package net.mcreator.freehandzsutilities.world.dimension;
...
import net.mcreator.freehandzsutilities.item.BedrockDimensionItem;
import net.mcreator.freehandzsutilities.block.Bedrock_OresOreBlockBlock;
...
public class BedrockDimensionDimension extends FreehandzsUtilitiesModElements.ModElement {
	@ObjectHolder("freehandzs_utilities:bedrock_dimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("freehandzs_utilities:bedrock_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public BedrockDimensionDimension(FreehandzsUtilitiesModElements instance) {
		super(instance, 23);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("bedrock_dimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("freehandzs_utilities:bedrock_dimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("freehandzs_utilities:bedrock_dimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("freehandzs_utilities:bedrock_dimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("freehandzs_utilities:bedrock_biome")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new BedrockDimensionItem().setRegistryName("bedrock_dimension"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("bedrock_dimension_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = BedrockDimensionDimension.CustomPortalBlock.createPatternHelper(entity.world, new BlockPos(pos));
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == Bedrock_OresOreBlockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == Bedrock_OresOreBlockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Bedrock_OresOreBlockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "bedrock_dimension_portal",
	...
	public static class TeleporterDimensionMod extends Teleporter {
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag
											? Bedrock_OresOreBlockBlock.block.getDefaultState().getBlock().getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Bedrock_OresOreBlockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = BedrockDimensionDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
